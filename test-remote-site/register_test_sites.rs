// 注册测试站点到deployment_sites.sqlite的简单工具
...
fn main() -> Result<()> {
    ...
    // 验证插入
    println!("\n验证注册结果:");
    let mut stmt = conn.prepare("SELECT id, name, location FROM remote_sync_sites WHERE env_id = 'test-env-001'")?;
