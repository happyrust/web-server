...
mod tests {
    ...
    /// 测试完整的房间关系构建（仅针对特定 FRMW）
    #[tokio::test]
    #[ignore = "需要真实数据库连接，手动运行"]
    async fn test_build_room_relations_for_specific_frmw() -> Result<()> {
        println!("\n🏗️  测试完整房间关系构建（特定 FRMW）");
        println!("{}", "=".repeat(80));
        ...
        println!("   房间关键词: {:?}", db_option.get_room_key_word());
        println!("   Mesh 路径: {}", db_option.get_meshes_path().display());

        // 调用 build_room_relations
        println!("\n🔄 开始构建房间关系...");
        let start = Instant::now();
        
        let stats = crate::fast_model::room_model::build_room_relations(&db_option).await?;
        
        println!("\n✅ 房间关系构建完成!");
        println!("   总耗时: {:?}", start.elapsed());
        println!("   房间数量: {}", stats.total_rooms);
        println!("   面板数量: {}", stats.total_panels);
        println!("   构件数量: {}", stats.total_components);
        println!("   缓存命中率: {:.2}%", stats.cache_hit_rate * 100.0);

        // 验证特定管道是否被正确分配到房间
        let pipe_refno = RefnoEnum::from_str("24381/59217").expect("无效的管道 refno");
        println!("\n🔍 验证管道 {} 的房间分配...", pipe_refno);

        let check_sql = format!(
            r#"
            SELECT * FROM room_relate WHERE in = {}
            "#,
            pipe_refno.to_pe_key()
        );
        let room_relates: Vec<serde_json::Value> = SUL_DB.query_take(&check_sql, 0).await.unwrap_or_default();

        if room_relates.is_empty() {
            println!("   ⚠️  管道未被分配到任何房间");
        } else {
            println!("   ✅ 管道被分配到 {} 个房间", room_relates.len());
            for relate in &room_relates {
                println!("   - {:?}", relate);
            }
        }
