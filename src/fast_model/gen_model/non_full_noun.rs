...
async fn collect_use_cata_refnos_from_tree(
    roots: &[RefnoEnum],
    debug_whitelist: Option<&HashSet<RefnoEnum>>,
    ...
    for &root in roots {
        ...
        for &node in &subtree_nodes {
            ...
            if blocked.get(&node).copied().unwrap_or(false) {
                // 检查是否在 debug 白名单中
                if let Some(whitelist) = debug_whitelist {
                    if !whitelist.contains(&RefnoEnum::from(node)) {
                        continue;
                    }
                    // 白名单覆盖屏蔽，继续处理
                } else {
...
async fn process_gen_geos_data_chunks(
    ...
    while gen_model && let Some(target_refnos) = chunked_root_refnos.next() {
        ...
        let target_single_cata_map = if is_incr_update {
            ...
            // - 但如果节点在 debug_model_refnos 白名单中，即使被 BRAN/HANG 屏蔽也允许生成
            let debug_whitelist: Option<HashSet<RefnoEnum>> = db_option_arc
                .inner
                .debug_model_refnos
                .as_ref()
                .map(|refno_strs| {
                    refno_strs
                        .iter()
                        .filter_map(|r| RefnoEnum::try_from(r.trim()).ok())
                        .collect()
                });

            use_cata_refnos = collect_use_cata_refnos_from_tree(
                &single_refnos,
                debug_whitelist.as_ref(),
            )
            .await
            .unwrap_or_default();
                    ...
                    match aios_core::get_named_attmap(*refno).await {
                        ...
                        Ok(att_map) => {
                            ...
                            // 再检查是否有 SPRE 关系
                                ...
                                // 诊断：为什么会判定“缺少 CATR/SPRE”
                                // 说明：BRAN/HANG tubing 串联依赖子件的 CATR/SPRE（解析 ptset/ARRI/LEAV），
                                // 这里把关键信息在 debug-model 日志中打出来，便于从日志定位数据缺口点。
                                debug_model_debug!(
                                    "[DBG][CATR_SPRE_MISS] refno={} type={} name={} owner={} sesno={:?}",
                                    refno,
                                    att_map.get_type_str(),
                                    att_map.get_name_or_default(),
                                    att_map.get_owner(),
                                    att_map.sesno()
                                );
                                debug_model_debug!(
                                    "[DBG][CATR_SPRE_MISS] has_keys: CATR={} :CATR={} SPRE={} :SPRE={} ARRI={} LEAV={}",
                                    att_map.contains_key("CATR"),
                                    att_map.contains_key(":CATR"),
                                    att_map.contains_key("SPRE"),
                                    att_map.contains_key(":SPRE"),
                                    att_map.contains_key("ARRI"),
                                    att_map.contains_key("LEAV")
                                );
                                debug_model_debug!(
                                    "[DBG][CATR_SPRE_MISS] vals: ARRI={:?} LEAV={:?} CATR_foreign={:?} SPRE_foreign={:?}",
                                    att_map.get_i32("ARRI"),
                                    att_map.get_i32("LEAV"),
                                    att_map.get_foreign_refno("CATR"),
                                    att_map.get_foreign_refno("SPRE")
                                );
